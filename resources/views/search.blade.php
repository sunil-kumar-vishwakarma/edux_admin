...
@section('content')
    ...
    <style>
        ...
        .programs-container {
            display: flex;
            flex-wrap: wrap;
            /* Ensures cards wrap to the next line if the screen is too narrow */
            gap: 20px;
            /* Space between cards */
            justify-content: space-between;
    ...
    <div class="top-buttons">
        <a href="https://docs.google.com/forms/d/e/1FAIpQLScTHfp1xeeFx5XQ49BVw_wBt6QItwkcsBO9icILsjKLu-FXJw/viewform?usp=dialog"
            class="top-btn" target="_blank">
            Search Feedback
    ...
    <div>
        <div class="search-container-wrapper">
            <div class="search-container">
                ...
                <input type="text" class="search-bar" id="programSearch" placeholder="What would you like to study?" />
            </div>
        </div>
    ...
    <main>
        ...
        <div class="results">
            ...
            <div class="sort">
                {{-- <button class="settings-btn" onclick="showPopup()"><i class="fa-sharp fa-solid fa-gear"></i> Display
                    settings</button>

                <div class="popup-overlay" id="popup-overlay">
                    <div class="popup">
                        <h3 style="font-weight: bold;">Display Settings</h3><br>
                        <h6>You can adjust the following configurations to display additional information on the program
                            cards.</h6>
                ...
                <button class="sort-btn" id="sortToggle">Sort <i class="fa-solid fa-arrow-down-short-wide"></i></button>
                <div id="sortMenu" class="dropdown-content1">
                    <p>Sort</p>
            ...
            <div class="programs-container">
                @foreach ($programs as $program)
                    <div class="program-card">
                        ...
                        <h3>{{ $program->university_name }} - {{ $program->campus_city }} Campus</h3>

                        <div class="program-badges">
                            @if ($program->is_high_demand)
                        ...
                        <div class="program-details">
                            <small>{{ $program->certificate }}</small>
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="2"></td>
                                        <td>₹ {{ number_format($program->tuition) }} INR</td>
                                    </tr>
                                    <tr>
                                        <td scope="row">Application fee</td>
                                        <td colspan="2"></td>
                                        <td>₹ {{ number_format($program->application_fee) }} INR</td>
                                    </tr>
                                    <tr>
                        ...
                        <p>Success prediction: <span style="font-weight:500 ;">{{ $program->success_prediction }}</span>
                            <button class="success">Details</button>
                        </p>
            ...
            <div class="pagination">
                ...
                @else
                    <a href="{{ $programs->previousPageUrl() }}" class="page-btn prev-btn">Previous</a>
                @endif

                <div class="page-numbers">
                    @for ($i = 1; $i <= $programs->lastPage(); $i++)
                        <a href="{{ $programs->url($i) }}"
                            class="page-btn {{ $i == $programs->currentPage() ? 'active' : '' }}">
                            {{ $i }}
                        </a>
                    @endfor
                </div>
                ...
                @if ($programs->hasMorePages())
                    <a href="{{ $programs->nextPageUrl() }}" class="page-btn next-btn">Next</a>
                @else
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Sort handlers
            const programContainer = document.querySelector(".programs-container");

            function getValueFromCard(card, selector) {
                const td = card.querySelector(selector);
                if (!td) return 0;
                return parseFloat(td.textContent.replace(/[^\d.]/g, '')) || 0;
            }

            function sortPrograms(by, order = "asc") {
                const cards = Array.from(programContainer.querySelectorAll(".program-card"));
                cards.sort((a, b) => {
                    const valA = getValueFromCard(a, by);
                    const valB = getValueFromCard(b, by);
                    return order === "asc" ? valA - valB : valB - valA;
                });

                // Re-render in order
                cards.forEach(card => programContainer.appendChild(card));
            }

            // Click listeners for sort options
            sortMenu.querySelectorAll("a").forEach(link => {
                link.addEventListener("click", function(e) {
                    e.preventDefault();

                    if (this.textContent.includes("Tution Cost (Low to High)")) {
                        sortPrograms("tr:nth-child(3) td:last-child", "asc");
                    } else if (this.textContent.includes("Tution Cost (High to Low)")) {
                        sortPrograms("tr:nth-child(3) td:last-child", "desc");
                    } else if (this.textContent.includes("Application fee(Low to High)")) {
                        sortPrograms("tr:nth-child(4) td:last-child", "asc");
                    } else if (this.textContent.includes("Application fee(High to Low)")) {
                        sortPrograms("tr:nth-child(4) td:last-child", "desc");
                    } else {
                        // Default: restore original order (refresh page or add logic if needed)
    ...
    <script>
        document.getElementById('programSearch').addEventListener('input', function() {
            ...
            programs.forEach(program => {
                const content = program.textContent.toLowerCase();
